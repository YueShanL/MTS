...
class DataCollatorForMTSGen2WithWaveform:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        converted = [self.convert_example(f) for f in features]
...
@dataclass
class DataCollatorForMTSGen2ReinforceLearning:
    pad_token_id: int = 0
    label_pad_token_id: int = -100

    def convert_example(self, example: Dict[str, Any]) -> Dict[str, Any]:
        # 1. 处理音频（同之前）
        audio = torch.tensor(example["audio"], dtype=torch.float32).unsqueeze(0)
        midi_bytes = example["midi_bytes"]
        midi = None
        if isinstance(midi_bytes, bytes):
            midi_obj = pretty_midi.PrettyMIDI(io.BytesIO(midi_bytes))
            midi = midi_obj
        orig_sr = example['audio_sampling_rate']
        target_sr = 22050
        if orig_sr != target_sr:
            resampler = torchaudio.transforms.Resample(orig_sr, target_sr)
            audio = resampler(audio)
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        converted = [self.convert_example(f) for f in features]
        waveforms = [f["waveform"] for f in converted]
        midis = [f["midi"] for f in converted]
